...
function openMarketCallEditor(visitId, callId) {
  const visit = marketVisits.find((item) => item.id === visitId);
  const call = visit?.calls.find((item) => item.id === callId);
  if (!visit) return;
  ...
  marketCallEditContext = { visitId, callId, values };
  ...
  dialog.innerHTML = `<form class="note-form" id="marketCallEditForm">
    <div class="form-header"><div><p class="eyebrow">${escapeHtml(visit.name)}</p><h2>${call.kind === "appointment" ? "Edit appointment" : "Edit sales call"}</h2></div><button class="icon-button" type="button" data-cancel-call-edit aria-label="Close">&times;</button></div>
    ...
    <div class="form-actions"><button class="ghost-action" type="button" data-cancel-call-edit>Cancel</button><button class="primary-action" type="submit">Save changes</button></div>
  </form>`;
