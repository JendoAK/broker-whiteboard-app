...
function renderPreciseMarketWeek(visit) {
  ...
  const pixelsPerMinute = 52 / 60;
  const height = (end - start) * pixelsPerMinute;
  const hours = Array.from({ length: (end - start) / 60 }, (_, index) => start / 60 + index);
  return `<div class="precise-week-scroll"><div class="precise-week-grid" style="--week-height:${height}px">
